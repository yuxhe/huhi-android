...
const createWallet = (state: Rewards.State) => {
  ...
  chrome.send('huhi_rewards.saveSetting', ['enabledMain', 'true'])
  chrome.send('huhi_rewards.saveSetting', ['enabledContribute', 'true'])
...
const fetchRewardsInfo = (state: Rewards.State) => {
  ...
  chrome.send('huhi_rewards.getWalletProperties', [])
  chrome.send('huhi_rewards.getPendingContributionsTotal')
  chrome.send('huhi_rewards.getBalanceReports')
  ...
  if (!state.safetyNetFailed) {
    chrome.send('huhi_rewards.getGrants', ['', ''])
  }
}
...
const walletReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  switch (action.type) {
    ...
    case types.RECOVER_WALLET:
      ...
      chrome.send('huhi_rewards.recoverWallet', [action.payload.key])
      break
