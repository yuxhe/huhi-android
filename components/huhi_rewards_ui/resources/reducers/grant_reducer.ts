...
const grantReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  switch (action.type) {
    ...
    case types.GET_GRANT_CAPTCHA:
      ...
      chrome.send('huhi_rewards.getGrantCaptcha', [currentGrant.promotionId, currentGrant.type])
      break
    ...
    case types.SOLVE_GRANT_CAPTCHA: {
      ...
      if (promotionId && action.payload.x && action.payload.y) {
        chrome.send('huhi_rewards.solveGrantCaptcha', [JSON.stringify({
          x: action.payload.x,
          y: action.payload.y
        }), promotionId])
      }
    ...
    case types.ON_GRANT_FINISH: {
      ...
      switch (properties.status) {
        ...
        case 6:
          ...
          chrome.send('huhi_rewards.getGrantCaptcha', [])
          break
        case 13:
