...
def transifex_name_from_filename(source_file_path, filename):
  ...
  elif 'huhi-extension' in source_file_path:
...
def check_for_chromium_upgrade_extra_langs(src_root, grd_file_path):
  """Checks the Huhi GRD file vs the Chromium GRD file for extra languages."""
  ...
  assert len(x_huhi_extra_langs) == 0, ('Huhi GRD %s has extra languages %s over Chromium GRD %s' %
            (grd_file_path, chromium_grd_file_path, list(x_huhi_extra_langs)))
  x_chromium_extra_langs = chromium_langs - huhi_langs
  assert len(x_chromium_extra_langs) == 0, ('Chromium GRD %s has extra languages %s over Huhi GRD %s' %
...
def check_for_chromium_missing_grd_strings(src_root, grd_file_path):
  """Checks to make sure Huhi GRD file vs the Chromium GRD has the same amount of strings."""
  chromium_grd_file_path = get_original_grd(src_root, grd_file_path)
  if not chromium_grd_file_path:
    return
  grd_strings = get_grd_strings(grd_file_path)
  chromium_grd_strings = get_grd_strings(chromium_grd_file_path)
  huhi_strings = { string_name for (string_name, message_value, string_fp) in grd_strings }
  chromium_strings = { string_name for (string_name, message_value, string_fp) in chromium_grd_strings }
  x_huhi_extra_strings = huhi_strings - chromium_strings
  assert len(x_huhi_extra_strings) == 0, ('Huhi GRD %s has extra strings %s over Chromium GRD %s' %
            (grd_file_path, chromium_grd_file_path, list(x_huhi_extra_strings)))
  x_chromium_extra_strings = chromium_strings - huhi_strings
  assert len(x_chromium_extra_strings) == 0, ('Chromium GRD %s has extra strings %s over Huhi GRD %s' %
