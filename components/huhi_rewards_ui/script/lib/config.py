...
DIST_URL = 'https://huhi-huhi-binaries.s3.amazonaws.com/releases/'
HUHI_CORE_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
HUHI_BROWSER_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..'))
...
def product_name():
  return os.environ.get('npm_config_huhi_product_name') or huhi_core_package()['name'].split('-')[0]


def project_name():
  return os.environ.get('npm_config_huhi_project_name') or huhi_core_package()['name'].split('-')[0]


def get_chrome_version():
  version = os.environ.get('npm_config_huhi_version') or huhi_browser_package()['version']
...
def get_raw_version():
  return os.environ.get('npm_config_huhi_version') or huhi_browser_package()['version']
...
def get_chromedriver_version():
  ...
  build_gn_path = os.path.join(HUHI_CORE_ROOT, 'BUILD.gn')
...
def get_env_var(name):
  return os.environ.get('HUHI_' + name) or os.environ.get('npm_config_HUHI_' + name, '')
...
def s3_config():
  ...
  message = ('Error: Please set the $HUHI_S3_BUCKET, '
             '$HUHI_S3_ACCESS_KEY, and '
             '$HUHI_S3_SECRET_KEY environment variables')
