...
#define HUHI_UA "Huhi"
#define DDG_HUHI_UA "Huhi Chrome"
...
std::string BuildUserAgentFromOSAndProduct(const std::string& os_info,
                                           const std::string& product,
                                           const std::string& strHost) {
  std::string productToPass = AddHuhiUserAgentForDDG(product, strHost);
  //std::string huhiUA = MakeHuhiUserAgent(product);
...
std::string AddHuhiUserAgentForDDG(const std::string& baseString, const std::string& strHost) {
  if (baseString.find(DDG_HUHI_UA) != std::string::npos) {
    return baseString;
  }
  ...
  if (0 == iDDGPosition || (int)std::string("https://").length() == iDDGPosition) {
    ...
    if (iPos != -1) {
      res.replace(iPos, strlen(CHROME_UA), DDG_HUHI_UA);
    }
  }
  return res;
}

std::string MakeHuhiUserAgent(const std::string& baseString) {
  return std::string(HUHI_UA) + "/" + version_info::GetMajorVersionNumber();
}
