...
public class BookmarkModel extends BookmarkBridge {
    ...
    void moveBookmarks(List<BookmarkId> bookmarkIds, BookmarkId newParentId) {
        ...
        for (int i = 0; i < bookmarkIds.size(); ++i) {
            if (null != app && null != app.mHuhiSyncWorker && app.mHuhiSyncWorker.SyncBookmarkModelIsReady()) {
                app.mHuhiSyncWorker.SyncedMoveBookmark(bookmarkIds.get(i), newParentId);
            } else {
                moveBookmark(bookmarkIds.get(i), newParentId, appenedIndex + i);
            }
            bookmarksToMove[i] = getBookmarkById(bookmarkIds.get(i));
        }
        if (null != app && null != app.mHuhiSyncWorker && null != newParentId) {
            app.mHuhiSyncWorker.CreateUpdateDeleteBookmarks(HuhiSyncWorker.UPDATE_RECORD, bookmarksToMove, true, false);
        }
    }

    /**
     * Calls {@link BookmarkBridge#moveBookmark(BookmarkId, BookmarkId, int)} for the given
     * bookmark. The bookmark is appended at the end. Call that method from Huhi's sync only
     */
