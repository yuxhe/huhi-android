...
class AppMenu implements OnItemClickListener, OnKeyListener {
    ...
    void show(Context context, final View anchorView, boolean isByPermanentButton,
        ...
        for (int i = 0; i < numItems; ++i) {
            ...
            if (item.getItemId() == R.id.huhi_rewards_id &&
                (!ChromeFeatureList.isEnabled(ChromeFeatureList.HUHI_REWARDS) ||
                PrefServiceBridge.getInstance().isSafetynetCheckFailed())) {
                continue;
            }
            if (item.getItemId() == R.id.huhi_set_default_browser &&
                HuhiSetDefaultBrowserNotificationService.isHuhiSetAsDefaultBrowser(context)) {
