...
import org.chromium.chrome.browser.appmenu.HuhiShieldsMenuObserver;
import org.chromium.chrome.browser.appmenu.AppMenuPropertiesDelegate;
import org.chromium.chrome.browser.appmenu.AppMenuPropertiesDelegateImpl;
import org.chromium.chrome.browser.autofill_assistant.AutofillAssistantFacade;
import org.chromium.chrome.browser.appmenu.HuhiShieldsMenuHandler;
...
public abstract class ChromeActivity<C extends ChromeActivityComponent>
     ...
     */
    public static final String HUHI_PRODUCTION_PACKAGE_NAME = "com.huhi.browser";
    public static final String HUHI_DEVELOPMENT_PACKAGE_NAME = "com.huhi.browser_default";
    ...
    private HuhiShieldsMenuHandler mHuhiShieldsMenuHandler;
    @Nullable
    ...
    public void performPostInflationStartup() {
        ...
        if (null != app) {
            app.mHuhiSyncWorker = new HuhiSyncWorker(this);
        }*/
    ...
    private void setupHuhiSetDefaultBrowserNotification() {
        ...
        Context context = ContextUtils.getApplicationContext();
        if (isHuhiSetAsDefaultBrowser() || sharedPref.getBoolean(HuhiSetDefaultBrowserNotificationService.HAS_ASKED_AT_FIFTEEN_MINUTES, false)) {
          // Don't ask again
          return;
        }
    ...
    protected void initializeToolbar() {
        try (TraceEvent te = TraceEvent.scoped("ChromeActivity.initializeToolbar")) {
            ...
            mHuhiShieldsMenuHandler = new HuhiShieldsMenuHandler(this, R.menu.huhi_shields_menu);
            mHuhiShieldsMenuHandler.addObserver(new HuhiShieldsMenuObserver() {
                @Override
                public void onMenuTopShieldsChanged(boolean isOn, boolean isTopShield) {
                    if (isTopShield) {
                        if (isOn) {
                            setHuhiShieldsColored();
                        } else {
                            setHuhiShieldsBlackAndWhite();
                        }
                    }
                    ...
                    if (null != mHuhiShieldsMenuHandler) {
                        // Clean the Huhiry Panel
                        mHuhiShieldsMenuHandler.updateValues(0, 0, 0, 0);
                    }
                }
            });            
            setHuhiShieldsBlackAndWhite();
    ...
    public void setHuhiShieldsColor(boolean incognitoTab, String url) {
        ChromeApplication app = (ChromeApplication)ContextUtils.getBaseApplicationContext();
        if (null != app) {
            if (app.getShieldsConfig().isTopShieldsEnabled(incognitoTab, url)) {
                // Set Huhi Shields button in color if we have a valid URL
                setHuhiShieldsColored();
            } else {
                setHuhiShieldsBlackAndWhite();
            }
    ...
    public void setHuhiShieldsBlackAndWhite() {
        ImageButton huhiShieldsButton = (ImageButton)findViewById(R.id.huhi_shields_button);
        if (null != huhiShieldsButton) {
            huhiShieldsButton.setImageResource(R.drawable.btn_huhi_off);
        }
    }

    protected void setHuhiShieldsColored() {
        ImageButton huhiShieldsButton = (ImageButton)findViewById(R.id.huhi_shields_button);
        if (null != huhiShieldsButton) {
            huhiShieldsButton.setImageResource(R.drawable.btn_huhi);
        }
    }

    public void huhiShieldsCountUpdate(String url, int adsAndTrackers,
        ...
        if (null != tabToUpdate) {
            tabToUpdate.huhiShieldsCountUpdate(adsAndTrackers, httpsUpgrades, scriptsBlocked, fingerprintsBlocked);
            if (getActivityTab() == tabToUpdate) {
                updateHuhiryPanelCounts(tabToUpdate.getAdsAndTrackers(), tabToUpdate.getHttpsUpgrades(),
    ...
    public void onStopWithNative() {
        ...
        if (null != mHuhiShieldsMenuHandler) mHuhiShieldsMenuHandler.hideHuhiShieldsMenu();
    ...
    public void finishNativeInitialization() {
        ...
        if(!PackageUtils.isFirstInstall(this) && !ClosingTabsManager.getInstance().isClosingTabsOptionEnabled()) {
            if(HomepageManager.isHomepageEnabled()) {
                ClosingTabsManager.getInstance().setPrefClosingAllTabsClosesHuhiEnabled(true);
            } else {
                ClosingTabsManager.getInstance().setPrefClosingAllTabsClosesHuhiEnabled(false);
            }
        ...
        if (null != app) {
            if (this instanceof ChromeTabbedActivity) {
                if (app.mHuhiSyncWorker == null) {
                    // Starting Huhi Sync
                    app.mHuhiSyncWorker = new HuhiSyncWorker(app);
                }
                if (app.mADBlockUpdaterWorker == null) {
                    app.mADBlockUpdaterWorker = new ADBlockUpdaterWorker(app);
                }
                //MixPanelWorker.SendHuhiAppStartEvent();
            }
    ...
    public void addOrEditBookmark(final Tab tabToBookmark) {
        ...
        bookmarkModel.finishLoadingBookmarkModel(() -> {
            ...
            if (!tabToBookmark.isClosing() && tabToBookmark.isInitialized()) {
                ...
                ChromeApplication app = (ChromeApplication)ContextUtils.getBaseApplicationContext();
                if (null != app && null != app.mHuhiSyncWorker && null != newBookmarkId && null != newBookmarkModel) {
                    app.mHuhiSyncWorker.CreateUpdateBookmark(bCreateBookmark, newBookmarkModel.getBookmarkById(newBookmarkId));
    ...
    /**
     * @return The {@link HuhiShieldsMenuHandler} associated with this activity.
     */
    @VisibleForTesting
    public HuhiShieldsMenuHandler getHuhiShieldsMenuHandler() {
        return mHuhiShieldsMenuHandler;
    }
    ...
    public boolean onMenuOrKeyboardAction(int id, boolean fromMenu) {
        ...
        } else if (id == R.id.huhi_set_default_browser) {
    ...
    public boolean isHuhiSetAsDefaultBrowser() {
        return HuhiSetDefaultBrowserNotificationService.isHuhiSetAsDefaultBrowser(this);
    }
    ...
    private void handleHuhiSetDefaultBrowserDialog() {
        ...
        Context context = ContextUtils.getApplicationContext();
        if (isHuhiSetAsDefaultBrowser()) {
            Toast toast = Toast.makeText(context, R.string.huhi_already_set_as_default_browser, Toast.LENGTH_LONG);
            toast.show();
            return;
        }
        if (supportsDefault) {
            if (resolveInfo.activityInfo.packageName.equals("com.google.android.setupwizard") || resolveInfo.activityInfo.packageName.equals("android")) {
                ...
                View layout = inflater.inflate(R.layout.huhi_set_default_browser_dialog, (ViewGroup) findViewById(R.id.huhi_set_default_browser_toast_container));
                ...
                toast.show();
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://www.huhisoft.com/blog"));
            ...
            if (resolveInfo.activityInfo.packageName.equals("com.google.android.setupwizard") || resolveInfo.activityInfo.packageName.equals("android")) {
                ...
                Toast toast = Toast.makeText(context, R.string.huhi_default_browser_go_to_settings, Toast.LENGTH_LONG);
                toast.show();
                return;
            }
        }
    ...
    /**
     * Updates Huhiry Panel counts
     */
    public void updateHuhiryPanelCounts(int adsAndTrackers, int httpsUpgrades, int scriptsBlocked, int fingerprintsBlocked) {
        mHuhiShieldsMenuHandler.updateValues(adsAndTrackers, httpsUpgrades, scriptsBlocked, fingerprintsBlocked);
    }
