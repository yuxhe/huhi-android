        ...
        countDownTimer = new CountDownTimer(endTime * 1000, 100) {
            ...
            public void onFinish() {
                ...
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        HuhiRewardsHelper.crossfade(null, actionLayout, View.GONE, 1f, HuhiRewardsHelper.CROSS_FADE_DURATION);
                        if (!fromSettings)
                            HuhiRewardsHelper.crossfade(countDownLayout, null, View.GONE, 1f, HuhiRewardsHelper.CROSS_FADE_DURATION);
                        else
                            countDownLayout.setVisibility(View.GONE);
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                HuhiRewardsHelper.crossfade(null, btnDidntSeeAd, View.GONE, 1f, HuhiRewardsHelper.CROSS_FADE_DURATION);
                            }
                        }, 2000);
