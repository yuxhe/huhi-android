...
import org.chromium.chrome.browser.preferences.HuhiSyncScreensObserver;
...
@JNINamespace("huhi_sync_storage")
public class HuhiSyncWorker {
    ...
    public static final String PREF_SYNC_TABS = "huhi_sync_tabs";
    public static final String PREF_SYNC_HISTORY = "huhi_sync_history";
    public static final String PREF_SYNC_AUTOFILL_PASSWORDS = "huhi_sync_autofill_passwords";
    public static final String PREF_SYNC_PAYMENT_SETTINGS = "huhi_sync_payment_settings";
    ...
    //private String mServerUrl = "https://sync-staging.huhisoft.com";
    private String mServerUrl = "https://sync.huhisoft.com";
    ...
    public final BookmarkModelObserver mBookmarkModelObserver = new BookmarkModelObserver() {
        ...
        @Override
        public void huhiExtensiveBookmarkChangesBeginning() {
        ...
        @Override
        public void huhiExtensiveBookmarkChangesEnded() {
            if (null != mBridge && !mBridge.isBookmarkModelLoaded()) {
                mBridge = null;
            }
            if (null == mBridge) {
        ...
        @Override
        public void huhiBookmarkModelLoaded(BookmarkBridge bridge) {
            mBridge = bridge;
        }
    };
    ...
    public HuhiSyncWorker(Context context) {
        mStopThread = false;
        mSeed = null;
        mDeviceId = null;
        mDeviceName = null;
    ...
    private void FetchSyncRecords(String lastRecordFetchTime, String category) {
        synchronized (mSyncThread) {
            ...
            if (0 == mTimeLastFetch && 0 == mTimeLastFetchExecuted) {
                ...
                try {
                    Thread.sleep(HuhiSyncWorker.INTERVAL_TO_FETCH_RECORDS);
        ...
        public void run() {
            if (null != mBookmarkModelObserver) {
                if (!mExtensiveOperation) {
                    mBookmarkModelObserver.huhiExtensiveBookmarkChangesEnded();
                } else {
                    mBookmarkModelObserver.huhiExtensiveBookmarkChangesBeginning();
                }
            }
        ...
        public void run() {
            ...
            for (;;) {
                try {
                    ...
                    for (int i = 0; i < HuhiSyncWorker.SYNC_SLEEP_ATTEMPTS_COUNT; i++) {
                        if (i == HuhiSyncWorker.SYNC_SLEEP_ATTEMPTS_COUNT / 2) {
                        ...
                        Thread.sleep(HuhiSyncWorker.INTERVAL_TO_FETCH_RECORDS / HuhiSyncWorker.SYNC_SLEEP_ATTEMPTS_COUNT);
    ...
    class SendSyncDataThread extends Thread {
        ...
        public void run() {
            for (;;) {
                try {
                    Thread.sleep(HuhiSyncWorker.INTERVAL_TO_SEND_SYNC_RECORDS);
    ...
    public void InitJSWebView(HuhiSyncScreensObserver syncScreensObserver) {
        try {
            synchronized (mSyncIsReady) {
