...
void NotificationPlatformBridgeAndroid::Display(
  ...
  // origin_url relates to GURL("chrome://huhi_ads/?" + *notification_id)
...
void NotificationPlatformBridgeAndroid::Close(
  ...
  //we might want to close Huhi ads notification from `bat-native-ads` service
  //between sessions and regenerated_notification_infos_ possibly was purged
  //already, so we need a way to identify a Huhi ad.
  std::string huhiAdsUrlPrefix(huhi_ads::kHuhiAdsUrlPrefix);
  std::string local_notification_id;

  bool huhi_ads_notification = (std::string::npos != notification_id.find(huhiAdsUrlPrefix));
  if (huhi_ads_notification){
    local_notification_id = notification_id.substr(huhiAdsUrlPrefix.size());
  }
  else{
  ...
  if (iterator == regenerated_notification_infos_.end()) {
    if (!huhi_ads_notification){
      ...
      //it's a Huhi ad and regenerated_notification_infos_ info was lost.
      //remove last parameters separator
      std::string huhi_pref = huhiAdsUrlPrefix.substr(0, huhiAdsUrlPrefix.size()-1);
