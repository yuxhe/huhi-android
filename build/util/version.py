...
def BuildParser():
  ...
  parser.add_argument('-bcn', '--huhi_correction_number', default=None,
                      help='Huhi correction number.')
...
def GenerateValues(options, evals):
  ...
  if options.os == 'android':
    values['HUHI_CORRECTION_NUMBER'] = '0'
    if options.huhi_correction_number is not None:
      values['HUHI_CORRECTION_NUMBER'] = options.huhi_correction_number
